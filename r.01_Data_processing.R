...
for (i in seq_along(sample_dirs)) {
	...
	# 计算线粒体基因比例http://127.0.0.1:40921/graphics/786fa85e-f05c-4b98-af79-05ce93ab5355.png
...
# 双峰检测 (DoubletFinder)
sweep.res <- paramSweep_v3(filtered_seurat, PCs = 1:15, sct = FALSE)
...
# 可视化PCA结果
pca_plot <- DimPlot(filtered_seurat, reduction = "pca", group.by = "sample") +
	theme_minimal() +
...
# UMAP可视化
umap_plot <- DimPlot(filtered_seurat, reduction = "umap", label = TRUE, pt.size = 0.5) +
	theme_minimal() +
...
# 可视化注释结果
annotated_umap <- DimPlot(filtered_seurat, group.by = "celltype", label = TRUE, repel = TRUE) +
...
fibroblast_subset <- RunUMAP(fibroblast_subset, dims = 1:15)

# 成纤维细胞UMAP图
fib_umap <- DimPlot(fibroblast_subset, reduction = "umap", label = TRUE) +
	theme_minimal() +
...
# 创建 CellChat 对象
cellchat <- createCellChat(object = expr, meta = cell_metadata, group.by = "celltype")
